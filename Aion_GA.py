...
PROC_NAME = "aion.bin"
MOD_NAME = "Game.dll"
POINTER_BASE = 0x010AF5C8 # 맵 이동 시 N/A 방지를 위한 베이스 주소
                ...
                if found_pid:
                    try:
                        ...
                        if mod:
                            ...
                            self.log_signal.emit(f"🎯 {PROC_NAME} 연결 성공.")
                    except: self.is_connected = False
            else:
                try:
                    if not psutil.pid_exists(self.target_pid): raise Exception()
                    # 로직 실행 중 에러가 나면(N/A 발생 시) 자동으로 재연결을 유도함
                    if not self.execute_logic():
                        self.is_connected = False # 맵 이동 등으로 인한 끊김 대응
                except:
